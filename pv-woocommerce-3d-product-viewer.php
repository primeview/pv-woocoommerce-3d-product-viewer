<?php
/**
 * Plugin Name: Primeview Woocommerce 3D Product Viewer
...
function pv_3dviewer_correct_filetypes( $data, $file, $filename, $mimes, $real_mime ) {
...
add_filter( 'wp_check_filetype_and_ext', 'pv_3dviewer_correct_filetypes' , 10, 5 );


// Adding Babylon Viewer into header
function pv_3dviewer_call()
{
    /*
...
add_action( 'wp_enqueue_scripts', 'pv_3dviewer_call' );
add_action( 'admin_enqueue_scripts', 'pv_3dviewer_call' );

// Adding Babylon Viewer shortcode
function pv_3dviewer_shortcode($atts = [], $content = null) {
...
add_shortcode('pv3dviewer', 'pv_3dviewer_shortcode');
//add_shortcode('pv3dviewer', 'pv_3dviewer_product_view');
...
// Create metabox
function pv_3dviewer_product_metabox(){
    add_meta_box(
        'pv_3dviewer_product_model_url',
        'PV 3D Viewer Model',
        'pv_3dviewer_product_metabox_callback',
        'product',
        'side',
        'high'
    );
}
add_action('add_meta_boxes', 'pv_3dviewer_product_metabox');

function pv_3dviewer_product_metabox_callback($post){
    wp_nonce_field(basename(__FILE__), 'pv_3dviewer_product_nonce');
    $pv_3dviewer_product_stored_meta = get_post_meta($post->ID);
    ?>
    ...
    <div>
        <div class="meta-row">
            ...
            <div class="meta-td">
                <input type="text" name="pv-babylon-product-url" id="pv-babylon-product-url" value="<?php if(!empty($pv_3dviewer_product_stored_meta['pv-babylon-product-url'])) echo esc_attr($pv_3dviewer_product_stored_meta['pv-babylon-product-url'][0]); ?>" />

                <!-- https://ccpatiodev.primeview.com/wp-content/uploads/2024/06/3SM.obj -->
                <section style="max-height:350px;" id="pvProdModel-wrap">
                    <div id="pvProdModel"></div>
                </section>
            </div>
        </div>
    </div>
...
function pv_3dviewer_product_view($product_id) {
...
add_action('woocommerce_single_product_summary', 'pv_3dviewer_product_view');
//add_action('brizy_template_content', 'pv_3dviewer_product_view');
